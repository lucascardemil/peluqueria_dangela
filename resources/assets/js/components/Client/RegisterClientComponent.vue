<template>
    <form action="POST" v-on:submit.prevent="createClient" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input v-validate="'required|min:7|max:25'" :class="{ 'input': true, 'is-invalid': errors.has('rut') }"
                type="text" name="rut" class="form-control form-control-sm" v-model="newClient.rut">
            <p v-show="errors.has('rut')" class="text-danger">{{ errors.first('rut') }}</p>
        </div>
        <div class="form-group">
            <label for="name">Nombre*</label>
            <input v-validate="'required|min:6|max:190'" :class="{ 'input': true, 'is-invalid': errors.has('name') }"
                type="text" name="name" class="form-control form-control-sm" v-model="newClient.name">
            <p v-show="errors.has('name')" class="text-danger">{{ errors.first('name') }}</p>
        </div>
        <div class="form-group">
            <label for="email">Email*</label>
            <input v-validate="'required|min:6|max:150|email'" :class="{ 'input': true, 'is-invalid': errors.has('email') }"
                type="text" name="email" class="form-control form-control-sm" v-model="newClient.email">
        ...
        <div class="form-group">
            ...
            <input v-validate="'required|min:8'" :class="{ 'input': true, 'is-invalid': errors.has('phone') }" type="number"
        ...
        <div class="form-group">
            ...
            <input v-validate="'required'" :class="{ 'input': true, 'is-invalid': errors.has('birthdate') }" type="date"
                name="birthdate" class="form-control form-control-sm" v-model="newClient.birthdate">
            <p v-show="errors.has('birthdate')" class="text-danger">{{ errors.first('birthdate') }}</p>
        </div>
        <div class="form-group">
            <label for="sex">Sexo*</label>
            <select v-validate="'required'" :class="{ 'input': true, 'is-invalid': errors.has('sex') }" name="sex"
        ...
        <div class="form-group">
            ...
            <input v-validate="'required|mimes:jpeg,jpg,png|max:2048'"
                :class="{ 'input': true, 'is-invalid': errors.has('image') }" type="file" name="image" id="image"
                class="form-control form-control-sm" @change="fileChange({ evt: $event })">
